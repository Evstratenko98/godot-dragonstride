...
- Кнопка `Start Game` открывает экран выбора одиночного уровня с вариантами `sandbox` и `level_1`; отдельно можно перейти в Steam-лобби, открыть настройки или выйти.
...
- В одиночной игре уровень выбирается на отдельном экране по разрешённому `level_id`; каталог содержит тестовую песочницу `sandbox` и уровень `level_1`.
- Multiplayer-матч из Steam-лобби всегда фиксирует уровень `sandbox`, независимо от предыдущего одиночного выбора.
...
- Размер сетки production-прототипа `level_1` — 52 × 34 клетки.
- Поддерживается преобразование координат мира в координаты клетки и обратно.
- Проверяются границы сетки.
- Проходимость определяется наличием тайла в разрешённых слоях уровня; `level_1` использует отдельный слой для каждой визуальной зоны.
...
- Начальное состояние задаётся уровнем: `level_1` запускает пошаговый режим, а `sandbox` оставляет свободный режим.
...
|---|---|---|
| `MainMenu` | `scenes/menu/main_menu/` | Навигацию из главного меню к выбору уровня, lobby и settings. |
| `LevelSelection` | `scenes/menu/level_selection/` | Выбор `sandbox` или `level_1`, запуск одиночной сессии с выбранным `level_id`. |
...
- В `level_1` пошаговый режим автоматически запускается `MatchController` через системную запись action stream; в `sandbox` он включается командой `game_turns_enable`.
...
- `level_1` объединяет семь проходимых TileMap-слоёв зон; `Water` остаётся непроходимым фоном.
...
- Экран выбора уровня пока содержит только `sandbox` и `level_1`; multiplayer-лобби намеренно запускает только `sandbox`.
