...
- Кнопка `Start Game` запускает одиночную сессию в `sandbox`; отдельно можно перейти в Steam-лобби, открыть настройки или выйти.
...
- Каталог уровней содержит только тестовую песочницу `sandbox`; её используют одиночная игра и multiplayer-матч из Steam-лобби.
...
- Размер сетки задаётся конфигурацией уровня; `sandbox` использует значение 19 × 19 клеток.
- Поддерживается преобразование координат мира в координаты клетки и обратно.
- Проверяются границы сетки.
- Проходимость определяется наличием тайла в разрешённых слоях уровня; основной проходимый слой `sandbox` — `Ground`.
...
- По умолчанию `sandbox` работает в свободном режиме.
...
- Пошаговый режим не включается автоматически; в `sandbox` он включается командой `game_turns_enable`.
...
- Основная проходимая поверхность `sandbox` — TileMap-слой `Ground`; `Water` не считается проходимым.
...
- Каталог уровней пока содержит только `sandbox`; отдельный экран выбора уровня отсутствует.
