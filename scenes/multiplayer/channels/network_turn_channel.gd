...
signal turn_end_requested(match_id: String, turn_revision: int, request_id: int, requester_peer_id: int)
...
func request_turn_end(match_id: String, turn_revision: int, request_id: int) -> void:
	if not GameSession.is_multiplayer():
		turn_end_requested.emit(match_id, turn_revision, request_id, 0)
		return
	if not _can_send():
		return
	if connection.is_host:
		turn_end_requested.emit(match_id, turn_revision, request_id, 0)
		return
	rpc_id(1, "_submit_turn_end", match_id, turn_revision, request_id)
...
func _submit_turn_end(match_id: String, turn_revision: int, request_id: int) -> void:
	...
	if requester_peer_id != 0 and turn_revision >= 0 and _is_valid_intent(match_id, request_id, {"turn_revision": turn_revision}):
		turn_end_requested.emit(match_id, turn_revision, request_id, requester_peer_id)
