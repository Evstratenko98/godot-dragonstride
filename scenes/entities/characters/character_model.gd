...
func _unhandled_input(event: InputEvent) -> void:
	...
	if character.runtime.has_selected_spell(character):
		...
		get_viewport().set_input_as_handled()
		return
	match character.action_mode:
		PlayerCharacter.ActionMode.NONE:
			return
		...
		PlayerCharacter.ActionMode.SPECIAL_ABILITY:
			character.request_special_ability_surface(target_surface)
...
func _can_read_movement_input() -> bool:
	if not character.can_process_local_input() or _is_console_open():
		return false
