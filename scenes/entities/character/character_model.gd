...
func _process(_delta: float) -> void:
	...
	var can_read_movement_input := _can_read_movement_input()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if not character.can_receive_input or character.world == null or _is_console_open():
		return

	if event.is_action_pressed("end_turn"):
		character.world.request_end_turn(character)
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		character.request_attack_cell(character.world.world_to_cell(character.get_global_mouse_position()), true, true)
...
func _can_read_movement_input() -> bool:
	...
	if character.world != null and character.world.has_method("can_entity_move_in_turn"):
		return character.world.can_entity_move_in_turn(character)
